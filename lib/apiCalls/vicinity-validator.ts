import { fetchBusLocations } from './bus-location';
...
export async function validateVicinity(
  ...
  try {
    ...
    // Get all saved bus locations
    const savedLocations = await fetchBusLocations();
    
    if (savedLocations.length === 0) {
      // If no saved locations exist, allow any valid land coordinates
      console.warn('No saved bus locations found. Allowing all land coordinates.');
      return { isValid: true };
    }

    // Check if coordinates match any saved location (within 1km precision)
    const isExactMatch = savedLocations.some(location => {
    ...
    // Check if coordinates are within vicinity radius of any saved location
    const isWithinVicinity = savedLocations.some(location => {
      const distance = calculateDistance(lat, lng, location.latitude, location.longitude);
